/*
 *  Code for Japanの項目定義に概ね準拠したCSVデータのロードを行う。
 * 
 * 形式: CSV
 */
create or replace procedure UPDATE_PATIENTS(
...
begin
    ...
    (
        select
            ...
            normalize_date(col007) as "発症_年月日",
            normalize_text(col008) as "患者_居住地",
            normalize_age(col009)  as "患者_年代",
            normalize_sex(col010)  as "患者_性別",
            col011 as "患者_職業",
            col012 as "患者_状態",
            col013 as "患者_症状",
            normalize_flag(col014) as "患者_渡航歴の有無フラグ",
            col015 as "備考",
            normalize_flag(col016) as "患者_退院済フラグ"
        from table(
        ...
        select 
            ...
            "患者_症状","患者_渡航歴の有無フラグ","備考","患者_退院済フラグ"
