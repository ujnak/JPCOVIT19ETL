create or replace function parse_pdf_ehime(
  P_MUNICIPALITY_CODE IN NUMBER DEFAULT 380008,
  P_URL IN VARCHAR2,
...
begin
  ...
  loop
      ...
      if regexp_like(l_line, '^【\d+') then
      ...
      elsif regexp_like(l_line, '事例目】') then
          -- 今までのを出力する
          ...
          l_age := regexp_replace(l_age, '^\(1\)\s*年\s*代\s*(\w+)', '\1', 1,1);
          l_string := apex_t_varchar2();
          apex_string.push(l_string, l_line);
      elsif instr(l_line, '(3) 住所地') > 0 then
          -- 今まで出現していたのは性別
          l_sex := apex_string.join(l_string, null);
          l_sex := regexp_replace(l_sex, '^\(2\)\s*性\s*別\s*(\w+)', '\1', 1,1);
          l_string := apex_t_varchar2();
          apex_string.push(l_string, l_line);
      elsif instr(l_line, '(4) 職業') > 0 then
          l_home := apex_string.join(l_string, null);
          l_home := regexp_replace(l_home, '^\(3\)\s*住所地\s*(\w+)', '\1', 1,1);
          l_string := apex_t_varchar2();
          apex_string.push(l_string, l_line);
      elsif instr(l_line, '(5) 症状') > 0 then
          l_job := apex_string.join(l_string, null);
          l_job := regexp_replace(l_job, '^\(4\)\s*職業\s*(\w+)', '\1', 1,1); 
          l_string := apex_t_varchar2();
          apex_string.push(l_string, l_line);
      elsif instr(l_line, '入院') > 0 then
          apex_string.push(l_string, l_line);
