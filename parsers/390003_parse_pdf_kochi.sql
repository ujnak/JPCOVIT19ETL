create or replace function parse_pdf_kochi( 
  P_MUNICIPALITY_CODE IN NUMBER DEFAULT 390003, 
  P_URL IN VARCHAR2, 
...
begin 
  ...
  loop 
      ...
      if regexp_like(l_line, '^県内第\s*\d+\s*例目') then
          -- 今までのを出力する 
          ...
          l_patient_no := regexp_substr(l_line, '(\d+)', 1,1); 
          l_string := apex_t_varchar2();
      elsif regexp_like(l_line, '^県内第') then 
          l_string := apex_t_varchar2(); 
          ...
          l_string := apex_t_varchar2(); 
      elsif instr(l_line, '(1)年代') > 0 then 
          -- 年代が最初に現れるので、スタックをクリアする。 
          l_string := apex_t_varchar2(); 
          apex_string.push(l_string, l_line); 
      elsif instr(l_line, '(2)性別') > 0 then 
          -- 今まで出現していたのは年代 
          l_age := apex_string.join(l_string, null); 
          l_age := regexp_replace(l_age, '^\(1\)年代:\s*(\w+)', '\1', 1,1); 
          l_string := apex_t_varchar2(); 
          apex_string.push(l_string, l_line); 
      elsif instr(l_line, '(3)居住地') > 0 then 
          -- 今まで出現していたのは性別 
          l_sex := apex_string.join(l_string, null); 
          l_sex := regexp_replace(l_sex, '^\(2\)性別:\s*(\w+)', '\1', 1,1); 
          l_string := apex_t_varchar2(); 
          apex_string.push(l_string, l_line); 
      elsif instr(l_line, '(4)職業') > 0 then 
          l_home := apex_string.join(l_string, null); 
          l_home := regexp_replace(l_home, '^\(3\)居住地:\s*(\w+)', '\1', 1,1); 
          l_string := apex_t_varchar2(); 
          apex_string.push(l_string, l_line); 
      elsif instr(l_line, '2)症状') > 0 then 
          l_job := apex_string.join(l_string, null); 
          l_job := regexp_replace(l_job, '^\(4\)職業:\s*(\w+)', '\1', 1,1);  
          l_string := apex_t_varchar2(); 
          apex_string.push(l_string, l_line); 
      elsif instr(l_line, '3)行動歴') > 0 then 
